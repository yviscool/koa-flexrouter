...
export class Tree {
    ...
    incrementChildPrio(pos: number) {
        ...
        const prio = n.children[pos].priority;
        ...
        while (path.length >= n.path.length && path.slice(0, n.path.length).toLocaleLowerCase() == n.path.toLocaleLowerCase()) {
            ...
            if (path.length > 0) {
                ...
                if (!n.wildChild) {
                    ...
                    const r = path[0].toLocaleLowerCase();

                    for (let i = 0; i < n.indices.length; i++) {

                        const indice = n.indices.charAt(i);

                        if (r == indice.toLocaleLowerCase()) {

                            const { ciPath: out, found } = n.children[i].findCaseInsensitivePath(path, fixTrailingSlash);
